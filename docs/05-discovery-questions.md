...
If unanswered we assume: A folder is a project keyed by the customer's order number; it may hold one or more vendor POs; each line item belongs to exactly one vendor PO.  
...
If unanswered we assume: A folder opens when the customer's PO arrives; the office manager creates the record and attaches the customer PO.  
Priority: Before build.
...
If unanswered we assume: The system assigns the next number, continuing from the current highest, one per vendor PO; the customer's order number is free text.  
Priority: Before build.
...
If unanswered we assume: Invoices are produced outside the platform; Phase 1 records the invoice number, amount, date and the PDF. No invoice generation.  
Priority: Before build.
...
If unanswered we assume: Phase 1 ships quote tracking only (a quotes table and convert-to-folder); quote document generation is deferred.  
Priority: Before build.
...
If unanswered we assume: Some version of QuickBooks is in use; Phase 1 builds only the generic external reference and a CSV export, no live connection.  
Priority: Before build.
...
If unanswered we assume: PDFs on a Windows shared folder with inconsistent naming, roughly organized by customer order number; Phase 1 intake is manual upload plus a watched folder; migration is an assisted import in Phase 2; email intake later.  
Priority: Before Phase 2.
...
If unanswered we assume: Nothing is formally classified and no facility clearance exists; "classified" means CUI or ITAR-sensitive.  
Priority: Before build.
...
If unanswered we assume: Some folders contain CUI- or ITAR-marked vendor specs. Every document gets a marking set during human verification, defaulting to "unreviewed", and unreviewed is treated as controlled.  
Priority: Before build.
